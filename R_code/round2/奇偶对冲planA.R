# 采用1或是-1规则（符合+1）“反”：奇数1，不反；奇数-1，反判定规则（符合-1）
...
minus_quantize=function(x,y,n=n){
  r=matrix(rep(1,length(x)*n),ncol = length(x),nrow = n)
  for (i in 1:length(x)) {
    r[which(str_detect(y,x[i])),i]=-1 # 改为匹配时记为负1
  }
  return(r)
}
#  训练----------------------------------------------------
...
# 交集
{
  ...
  write.csv(op_formula_190_data_final,'result\\op_formula_190_data_final.csv',row.names = F)
}
...
op_formula=read.csv('result\\op_formula_190_data_final.csv',stringsAsFactors = F,header = F)
n=nrow(data)-8
quantize=function(x,y,n=n){
  r=matrix(rep(-1,length(x)*n),ncol = length(x),nrow = n)
  for (i in 1:length(x)) {
...
pred_unsatisfy=function(index,data=data,formula){
  ...
  for (m in 1:length(data)) {
    ...
    # 以及“反”之后的quantity
    {
      R9_quantity=quantize(formula[index,3],final_comb[,1],n)
      minus_R9_quantity=minus_quantize(formula[index,3],final_comb[,1],n)
      
      R7_quantity=quantize(formula[index,2],final_comb[,2],n)
      minus_R7_quantity=minus_quantize(formula[index,2],final_comb[,2],n)
      
      R5_quantity=quantize(formula[index,1],final_comb[,3],n)
      minus_R5_quantity=minus_quantize(formula[index,1],final_comb[,3],n)
...
unsatisfy_set_names=list(NULL,NULL,NULL,NULL,NULL,NULL)
...
data=read.csv('data\\data_27.csv',header = T)
unsatisfy_set_names=list(NULL,NULL,NULL,NULL,NULL,NULL)
for (i in 1:nrow(op_formula)) {
  tempt=pred_unsatisfy(i,data,op_formula)
  unsatisfy_set_names[i]=list(tempt[[2]])
