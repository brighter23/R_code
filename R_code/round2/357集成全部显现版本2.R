...
run=function(data=data,path=path,data_sort,choice=0){
  ...
  for (m in 1:length(data)) {
    ...
    if(choice==0){
      ...
      forth=ifelse(first<0,forth,-forth)
      pattern[seq(1,n_sumtemp,4),]=1
      pattern[seq(2,n_sumtemp,4),]=1
      pattern[seq(3,n_sumtemp,4),]=ifelse(first*second>0,1,2)
      pattern[seq(4,n_sumtemp,4),]=ifelse(first<0,1,2)
    }
    else if(choice==13){
      ...
      pattern[seq(1,n_sumtemp,4),]=1
      pattern[seq(2,n_sumtemp,4),]=1
      pattern[seq(3,n_sumtemp,4),]=ifelse(second<0,1,2)
      pattern[seq(4,n_sumtemp,4),]=ifelse(first<0,1,2)
    }
    else if(choice==131){
      second=-second
      third=ifelse(second>0,third,-third)# 注意这里的符号
      forth=ifelse(first<0,forth,-forth) 
      pattern[seq(1,n_sumtemp,4),]=1
      pattern[seq(2,n_sumtemp,4),]=1
      pattern[seq(3,n_sumtemp,4),]=ifelse(second>0,1,2)
