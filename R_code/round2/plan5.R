...
run=function(data=data,type=T){
  ...
  set.seed(1994)#设定随机数种子，一起运行产生几次都是同一组随机数
  # B 对用1，P对应0
  random_BP=as.factor(ifelse(rbinom(30,1,0.5)==1,'B','P'))
  lis=NULL# 存放结果
  for (m in 1:length(data)) {
    ...
    R9_quantity[seq(1,n,2),]=ifelse(random_BP==data[seq(9,n+8,2),m],1,-1)
    R7_quantity[seq(1,n,2),]=ifelse(random_BP==data[seq(9,n+8,2),m],1,-1)
    R5_quantity[seq(1,n,2),]=ifelse(random_BP==data[seq(9,n+8,2),m],1,-1)
...
# 3、预测---------------------------------------------
pred_unsatisfy_num=function(index,data=data,type=T){
  n=nrow(data)-8
  ...
  for (m in 1:length(data)) {
    ...
    R9_quantity=new_quantize(path = path[index,,drop=F],real_comb=final_comb,set=data[,m])[[1]]
    minus_R9_quantity=new_quantize(path =minus_path[index,,drop=F],real_comb=final_comb,set=data[,m])[[1]]
    
    
    R7_quantity=new_quantize(path = path[index,,drop=F],real_comb=final_comb,set=data[,m])[[2]]
    minus_R7_quantity=new_quantize(path =minus_path[index,,drop=F],real_comb=final_comb,set=data[,m])[[2]]
    
    R5_quantity=new_quantize(path = path[index,,drop=F],real_comb=final_comb,set=data[,m])[[3]]
    minus_R5_quantity=new_quantize(path =minus_path[index,,drop=F],real_comb=final_comb,set=data[,m])[[3]]
...
unsatisfy_num1=pred_unsatisfy_num(index=6,data=data,type=F)
unsatisfy_num2=pred_unsatisfy_num(index=3,data=data,type=T)#我的版本的预测
