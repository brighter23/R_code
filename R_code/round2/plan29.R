...
data_sort=function(data=data,m,path=path){
  ...
  { 
    ...
    for (i in seq(23,n,4)) {
      k=which(sequance1==i)
      real_comb=true_path(data[sequance1[(k-9):k],m])
  ...
  { 
    ...
    for (i in seq(24,n,4)) {
      k=which(sequance2==i)
      real_comb=true_path(data[sequance2[(k-9):k],m])
...
# 基础版：556
plan29_0=run(all_data,path,0);plan29_0[[3]]
# paln3反一反的形式：555
plan29_1=run(all_data,path,1);plan29_1[[3]]
# plan11的反一反：550
plan29_2=run(all_data,path,2);plan29_2[[3]]
# plan13的反一反：545
plan29_3=run(all_data,path,3);plan29_3[[3]]
# 新的反一反形式27-2：544
plan29_4=run(all_data,path,4);plan29_4[[3]]