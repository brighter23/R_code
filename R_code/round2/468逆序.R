...
{
  library(stringr)
  formula=read.csv2('data\\468R.csv',header = F,stringsAsFactors = F)
  R4A=formula[2:7,];R4B=formula[9:14,]
  R6A=formula[16:25,];R6B=formula[27:36,]
  R8A=formula[38:51,];R8B=formula[53:66,]
  Formula=function(R){
    R_=str_split(R,pattern = ',',simplify = T)
    R_=apply(R_[,1:3], 1, function(x)str_c(x,collapse = '-'))
  ...
  path=matrix(c(R4A,R6A,R8A, R4A,R6B,R8A, R4A,R6A,R8B, R4A,R6B,R8B, R4B,R6A,R8A,
                R4B,R6B,R8A, R4B,R6A,R8B, R4B,R6B,R8B),byrow = T,ncol = 3)
  colnames(path)=c('R4','R6','R8')
...
quantize=function(path,set){
  ...
  # 分BP的path_BP有6列：
  if(ncol(path)==6){
    r8=matrix(0,ncol = n_row_path,nrow = 1)
    for (j in 1:n_row_path) {
      # 因为根据数据的开头得BP来判定,逆序时都是根据第一个数据
      r8[1,j]=ifelse(set[1]=='B'|set[1]==1,ifelse(str_detect(real_comb[1,3],path[j,'B_R8']),1,-1),ifelse(str_detect(real_comb[1,3],path[j,'P_R8']),1,-1))
    }
    r6=matrix(0,ncol = n_row_path,nrow = 1)
    for (j in 1:n_row_path) {
      r6[1,j]=ifelse(set[1]=='B'|set[2]==1,ifelse(str_detect(real_comb[1,2],path[j,'B_R6']),1,-1),ifelse(str_detect(real_comb[1,2],path[j,'P_R6']),1,-1))
    }
    r4=matrix(0,ncol = n_row_path,nrow = 1)
    for (j in 1:n_row_path) {
      r4[1,j]=ifelse(set[1]=='B'|set[4]==1,ifelse(str_detect(real_comb[1,1],path[j,'B_R4']),1,-1),ifelse(str_detect(real_comb[1,1],path[j,'P_R4']),1,-1))
    }
  }
  else{
    r8=matrix(0,ncol = nrow(path),nrow = 1)
    for (j in 1:nrow(path)) {
      r8[1,j]=ifelse(str_detect(real_comb[1,3],path[j,'R8']),1,-1)
    }
    ...
    for (j in 1:nrow(path)) {
      r4[1,j]=ifelse(str_detect(real_comb[1,1],path[j,'R4']),1,-1)
    }
  }
  return(r8+r6+r4)
} 
# quantize(path,set=data[1:8,1])
...
# 从15手开始，都为逆序，计算一个data在8个formula下的“似然值”：
sort_allsequence_fifteen=function(data=data,m,path=path){
  n=nrow(data)
  # 第一手：逆序 奇数轨奇数手
  {
    first_hand=matrix(ncol = nrow(path))
    for (i in seq(15,n,4)) {
      first_hand_quantity=quantize(path,set = data[c(seq(i-2,i-14,-2),i),m])
  ...
  { 
    ...
    for (i in seq(16,n,4)) {
      second_hand_quantity=quantize(path,set = data[c(seq(i-2,i-14,-2),i),m])
  ...
  # 第三手：逆序 奇数轨偶数手
  {
    ...
    for (i in seq(17,n,4)) {
      third_hand_quantity=quantize(path,set = data[c(seq(i-2,i-14,-2),i),m])
  ...
  { 
    ...
    for (i in seq(18,n,4)) {
      forth_hand_quantity=quantize(path,set = data[c(seq(i-2,i-14,-2),i),m])
...
run=function(data=data,path=path,choice=1019,rolling=F){
  ...
  for (m in 1:length(data)) {
    if(m%%50==0)print(m)
    temp=sort_allsequence_fifteen(data,m,path)
  # 1）初始化变量，以记录指标：
    {
      first=temp[[1]] # 奇数轨奇数行
