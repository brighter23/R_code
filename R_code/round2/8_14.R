...
data400_800=cbind(data2,data3)
all_data=cbind(data378,data400_800)

{
  ...
  AB5=cbind(A51,B52)
  BA5=cbind(B51,A52)
  path=matrix(c(AB9,AB7,AB5, AB9,BA7,AB5, AB9,AB7,BA5, AB9,BA7,BA5, BA9,AB7,AB5, BA9,BA7,AB5, BA9,AB7,BA5, BA9,BA7,BA5),byrow = T,ncol = 6)
  colnames(path)=c('B_R9','P_R9','B_R7','P_R7','B_R5','P_R5')
  minus_path=matrix(c(BA9,BA7,BA5, BA9,AB7,BA5, BA9,BA7,AB5, BA9,AB7,AB5, AB9,BA7,BA5, AB9,AB7,BA5, AB9,BA7,AB5, AB9,AB7,AB5),byrow = T,ncol = 6)
  colnames(minus_path)=c('B_R9','P_R9','B_R7','P_R7','B_R5','P_R5')
}
c=c('data','data1','data378','data2','data3','data400_800','all_data','path','minus_path')
rm(list=(setdiff(a,c)))
gc()
...
run=function(data=data,path=path,minus_path=minus_path,seq){
  ...
  colnames(final_newsum_index)=c(paste0('formula',1:8))
  # 交集
  t=table(lis)
...
# plan 14:534
res2=run(data=all_data,path=path,minus_path=minus_path,seq = c(1,2,1))
# 计算负的个数
res2[[1]]
for (i in 1:8) {
...
# plan 15:
res3=run(data=all_data,path=path,minus_path=minus_path,seq = c(1,2,3))
# 计算负的个数
res3[[1]]
for (i in 1:8) {
...
# plan 16:
res4=run(data=all_data,path=path,minus_path=minus_path,seq = c(0,2,1))
# 计算负的个数
res4[[1]]
for (i in 1:8) {
  print(paste0('formula ',i,' 的新指标中负的个数:',sum(res4[[2]][,i]<0)))
}
...
unsatisfy_num1=pred_unsatisfy_num(index=7,data=all_data,path=path,minus_path=minus_path,seq=c(1,1,2))
