...
c=c('f5_final_formular','f6_final_formular','f7_final_formular','f8_final_formular','f9_final_formular','f10_final_formular')
a=ls()
rm(list=(setdiff(a,c)))
gc()
...
for (m in 1:length(data)) {
  ...
  R9_final_quantity=quantize(f9_final_formular,final_comb[,1],n)
  R7_final_quantity=quantize(f7_final_formular,final_comb[,2],n)
  R5_final_quantity=quantize(f5_final_formular,final_comb[,3],n)
  
  #改变偶数行的0的每列全变为 even_final_result[seq(2,n,2),,drop=F]---------------
  even_final_result=matrix(nrow =n)
  for (i in seq(2,n,2)) {
    condition1=((data[i+7,m]==data[i+8,m]))
    #condition2=((data[i+7,m]!=data[i+6,m])&(data[i+7,m]!=data[i+8,m]))
    even_final_result[i]=ifelse((condition1),-1,1)
  }
  
  R5_final_quantity[seq(2,n,2),]=even_final_result[seq(2,n,2),,drop=F]
  R9_final_quantity[seq(2,n,2),]=matrix(rep(0,ncol(R9_final_quantity)))
  R7_final_quantity[seq(2,n,2),]=matrix(rep(0,ncol(R7_final_quantity)))
...
op_formula=read.csv('result\\op_formula_190_data_oddeven.csv',stringsAsFactors = F,header = F)
n=nrow(data)-8
quantize=function(x,y,n=n){
  r=matrix(rep(-1,length(x)*n),ncol = length(x),nrow = n)
  for (i in 1:length(x)) {
...
pred_unsatisfy=function(index,data=data,formula){
  n=nrow(data)-8
  ...
  for (m in 1:length(data)) {
    ...
    f9_final_quantity=quantize(formula[index,3],final_comb[,1],n)
    f7_final_quantity=quantize(formula[index,2],final_comb[,2],n)
    f5_final_quantity=quantize(formula[index,1],final_comb[,3],n)
    even_final_result=matrix(nrow =n)
    for (i in seq(2,n,2)) {
      condition1=((data[i+7,m]==data[i+6,m])&(data[i+7,m]==data[i+8,m]))
      condition2=((data[i+7,m]!=data[i+6,m])&(data[i+7,m]!=data[i+8,m]))
      even_final_result[i]=ifelse((condition1|condition2),-1,1)
    }
    f5_final_quantity[seq(2,n,2),]=even_final_result[seq(2,n,2),,drop=F]
    f9_final_quantity[seq(2,n,2),]=matrix(rep(0,ncol(f9_final_quantity)))
    f7_final_quantity[seq(2,n,2),]=matrix(rep(0,ncol(f7_final_quantity)))
...
unsatisfy_set_names=list(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL)
for (i in 1:nrow(op_formula)) {
  tempt=pred_unsatisfy(i,data,op_formula)
  unsatisfy_set_names[i]=list(tempt[[2]])
  assign(paste0('error_formular_',i,'_final_result'),data.frame(tempt[[1]]))
}

unique(unsatisfy_set_names)
