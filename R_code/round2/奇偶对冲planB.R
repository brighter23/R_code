...
enen_change=function(quantity,minus_quantity){
  ...
  for (j in 1:ncol(quantity)) {
    for (i in seq(2,n,2)) {# 根据奇数行改变偶数行结果
...
# 定义反一反的formula
op_formula=read.csv('result\\op_formula_190_data_final2.csv',stringsAsFactors = F,header = F)
library(stringr)
# 必须有new_f5_final_formular以及f5_final_formular等
...
op_formula[3,]
opsite_formula(op_formula[3,])
# 3、planA 预测----------------------------------------------------------------


pred_unsatisfyA=function(index,data=data,old_formula,new_formula){
  n=nrow(data)-8
...
for (i in 1:nrow(op_formula)) {
  ...
  tempt=pred_unsatisfyA(1,data,op_formula[i,],opsite_formula(op_formula[i,]))
...
op_formula=read.csv('result\\op_formula_190_data_final2.csv',stringsAsFactors = F,header = F)
library(stringr)
pred_unsatisfyB=function(index,data=data,old_formula,new_formula){
  n=nrow(data)-8
  ...
  for (m in 1:length(data)) {
    ...
    for (l in 1:(n-9)){
      if(sumtemp[l]<0&sumtemp[l+1]<0&sumtemp[l+2]<0&sumtemp[l+3]<0&sumtemp[l+4]<0&sumtemp[l+5]<0&sumtemp[l+6]<0&sumtemp[l+7]<0&sumtemp[l+8]<0&sumtemp[l+8]<0){
        colnames(sumtemp)=c(paste0('data',m))
...
for (i in 1:nrow(op_formula)) {
  # 注意这里formula的循环已经体现在op_formula[i,]，因此不需要再index=i，而是恒定为1
  tempt=pred_unsatisfyB(1,data,op_formula[i,],opsite_formula(op_formula[i,]))
...
# 查看第2个formula的极端情况分布：dis_unsatisfy(error_formular_2_final_result[[1]])
