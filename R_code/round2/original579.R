...
{
  library(stringr)
  formula=read.csv2('data\\original579.csv',header = F,stringsAsFactors = F)
  
  R5A=formula[1:6,];R5B=formula[8:13,]
  R7A=formula[15:24,];R7B=formula[26:35,]
  R9A=formula[37:50,];R9B=formula[52:65,]
  ...
  R9A=Formula(R9A,c1 = 1:3,c2=4:6);R9B=Formula(R9B,c1 = 1:3,c2=4:6)
  
  path_BP_original579=matrix(c(R5A,R7A,R9A, R5A,R7B,R9A, R5A,R7A,R9B, R5A,R7B,R9B, R5B,R7A,R9A, R5B,R7B,R9A, R5B,R7A,R9B, R5B,R7B,R9B),byrow = T,ncol = 6)
  colnames(path_BP_original579)=c('B_R5','P_R5','B_R7','P_R7','B_R9','P_R9')
}

c=c('data','all_data','path_BP_original579')
a=ls()
rm(list=(setdiff(a,c)))
gc()
library(stringr)
...
# new_quantize(path = path_BP_original579,set=data[1:9,1],Inverse=F) # 测试应该是正负相对
# 2.3 定义诊断连续负的长度：
...
# 顺+不交叉：
plan1019201_original579=run(data = all_data,path = path_BP_original579,inverse = F,choice=1019201,rolling=T,reflection=F,cross = F)
present(plan1019201_original579)
# 顺序+交叉（twist）：
plan1019201_original579_cross=run(data = all_data,path = path_BP_original579,inverse = F,choice=1019201,rolling=T,reflection=F,cross = T)
present(plan1019201_original579_cross)