...
data_sort=function(data=data,m,path=path){
  ...
  # 第一手
  {
    ...
    for (i in seq(1,n,4)[1:9]) {
      real_comb=true_path(data[seq(i,i+32,4),m])
      first_hand_quantity=new_quantize(path,real_comb,set =data[seq(i,i+32,4),m] )
  ...
  # 第二手
  {
    ...
    for (i in seq(2,n,4)[1:9]) {
      real_comb=true_path(data[seq(i,i+32,4),m])
      second_hand_quantity=new_quantize(path,real_comb,set =data[seq(i,i+32,4),m] )
  ...
  # 第三手
  {
    ...
    for (i in seq(3,n,4)[1:9]) {
      real_comb=true_path(data[seq(i,i+32,4),m])
      third_hand_quantity=new_quantize(path,real_comb,set =data[seq(i,i+32,4),m] )
  ...
  # 第四手
  {
    ...
    for (i in seq(4,n,4)[1:9]) {
      real_comb=true_path(data[seq(i,i+32,4),m])
      forth_hand_quantity=new_quantize(path,real_comb,set =data[seq(i,i+32,4),m] )
...
run=function(data=data,path=path,seq){
  ...
  for (m in 1:length(data)) {
    ...
    if(seq==0){
      ...
      forth=ifelse(third*first>0,forth,-forth)
...
run(data = all_data,path=path,seq = 0)
run(data = all_data,path=path,seq = 1)
run(data = all_data,path=path,seq = 2)
run(data = all_data,path=path,seq = 3)