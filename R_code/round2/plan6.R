...
# 4、预测---------------------------------------------
pred_unsatisfy6=function(index,data=data,path){
  n=nrow(data)-8
  ...
  for (m in 1:length(data)) {
    ...
    R9_final_quantity=enen_change(R9_quantity,minus_R9_quantity)
    R9_final_quantity=R9_final_quantity[,index]
    R7_final_quantity=enen_change(R7_quantity,minus_R7_quantity)
    R7_final_quantity=R7_final_quantity[,index]
    R5_final_quantity=enen_change(R5_quantity,minus_R5_quantity)
    R5_final_quantity=R5_final_quantity[,index]
...
error1=pred_unsatisfy6(2,data,path = path )
error2=pred_unsatisfy6(3,data,path = path )
error3=pred_unsatisfy6(6,data,path = path )
