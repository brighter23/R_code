...
data_sort=function(data=data,m,path=path){
  ...
  # 第一手
  {
    ...
    for (i in seq(15,n,4)) {
      real_comb=true_path(data[(i-8):i,m])
      first_hand_quantity=new_quantize(path,real_comb,set =data[(i-8):i,m] )
      first_hand=rbind(first_hand,first_hand_quantity)
    }
    first_hand=first_hand[-1,]
    # colnames(first_hand)=c(paste0('formula.',1:8))
  }
  # 第二手
  {
    ...
    for (i in seq(16,n,4)) {
      real_comb=true_path(data[(i-8):i,m])
      second_hand_quantity=new_quantize(path,real_comb,set =data[(i-8):i,m] )
  ...
  # 第三手
  {
    ...
    for (i in seq(17,n,4)) {
      ...
      third_hand_quantity=new_quantize(path,real_comb,set =data[seq(i-16,i,2),m] )
  ...
  # 第四手
  {
    ...
    for (i in seq(18,n,4)) {
      real_comb=true_path(data[seq(i-16,i,2),m])
      forth_hand_quantity=new_quantize(path,real_comb,set =data[seq(i-16,i,2),m] )
...
run=function(data=data,path=path,seq){
  ...
  for (m in 1:length(data)) {
    ...
    if(seq==0){
      ...
      forth[seq(2,nrow(forth),2),]=ifelse(forth[seq(1,nrow(forth)-1,2),]<0,forth[seq(2,nrow(forth),2),],-forth[seq(2,nrow(forth),2),])
    }
    else if(seq==3){
      second=ifelse(first<0,second,-second)
      third=ifelse(first[-nrow(first),]<0,third,-third)
      forth=ifelse(third*first[-nrow(first),]>0,forth,-forth)
      
    }
    else if(seq==4){
      for (i in 3:nrow(first)){
        first[i,]=ifelse(first[i-1,]*first[i-2,]>0,first[i,],-first[i,])
      }
      second=ifelse(first>0,second,-second)
      for (i in 3:nrow(third)){
        third[i,]=ifelse(third[i-1,]*third[i-2,]>0,third[i,],-third[i,])
      }
      forth=ifelse(third<0,forth,-forth)
...
# 1(我版),3（他版）,自身反一反：变差
plan2=run(all_data,path,2);plan2[[3]]
# plan17基础上，1固定，2根据1，3根据1，4根据3：
plan3=run(all_data,path,3);plan3[[3]]
# 
plan4=run(all_data,path,4);plan4[[3]]
run(all_data,path,5)

# 3、预测============================================================

new_dis_unsatisfy=function(result,index){
    for(j in 4:nrow(result)){
      k=result[j,index]
      print(paste0('最大连续负为',j,'的数据集个数：',k))
    }
}

new_dis_unsatisfy(result = plan2[[2]],8)
new_dis_unsatisfy(result = plan3[[2]],5)
