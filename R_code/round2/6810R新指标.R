...
data_sort3=function(data=data,m,path=path){
  ...
  # 第一轨
  {
    ...
    for (i in 1:25) {
      ...
      first_hand[i,]=new_quantize(path,real_comb)
    }
  ...
  # 第二轨
  {
    ...
    for (i in 1:25) {
      ...
      second_hand[i,]=new_quantize(path,real_comb)
    }
...
run=function(data=data,path=path,seq){
  ...
  for (m in 1:length(data)) {
    ...
    if(seq==0){
      ...
      first[seq(2,nrow(first),2),]=ifelse(first[seq(1,nrow(first)-1,2),]<0,first[seq(2,nrow(first),2),],-first[seq(2,nrow(first),2),])
      second[seq(1,nrow(second),2),]=ifelse(first[seq(1,nrow(first),2),]<0,second[seq(1,nrow(second),2),],-second[seq(1,nrow(second),2),])
      second[seq(2,nrow(second),2),]=ifelse(first[seq(1,nrow(first)-1,2),]* second[seq(1,nrow(second)-1,2),]>0,second[seq(2,nrow(second),2),],-second[seq(2,nrow(second),2),])
      pattern[seq(1,n,4),]=1 # 奇数轨奇数行固定使用pattern A  
      pattern[seq(2,n,4),]=ifelse(first[seq(1,nrow(first),2),]<0,1,2) # 偶数轨奇数行根据奇数轨奇数行
      pattern[seq(3,n,4),]=ifelse(first[seq(1,nrow(first)-1,2),]<0,1,2) # 奇数轨偶数行根据奇数轨奇数行
      pattern[seq(4,n,4),]=ifelse(first[seq(1,nrow(first)-1,2),]* second[seq(1,nrow(second)-1,2),]>0,1,2) # 偶数轨偶数行根据 奇数行奇数轨、偶数轨奇数行共同决定
    }
    else if(seq==2){
      first[seq(2,nrow(first),2),]=ifelse(first[seq(1,nrow(first)-1,2),]<0,first[seq(2,nrow(first),2),],-first[seq(2,nrow(first),2),])
      second[seq(2,nrow(second),2),]=ifelse(second[seq(1,nrow(second)-1,2),]<0,second[seq(2,nrow(second),2),],-second[seq(2,nrow(second),2),])
      
      pattern[seq(1,n,4),]=1 # 奇数轨奇数行固定使用pattern A  
      pattern[seq(2,n,4),]=1 # 偶数轨奇数行固定使用pattern A 
      pattern[seq(3,n,4),]=ifelse(first[seq(1,nrow(first)-1,2),]<0,1,2) # 奇数轨偶数行根据奇数轨奇数行
      pattern[seq(4,n,4),]=ifelse(second[seq(1,nrow(second)-1,2),]<0,1,2) # 偶数轨偶数行根据偶数轨奇数行
    }
    else if(seq==3){
      ...
      pattern[seq(1,n,4),]=1 # 奇数轨奇数行固定使用pattern A  
      pattern[seq(2,n,4),]=2 # 偶数轨奇数行固定使用pattern B 
      pattern[seq(3,n,4),]=ifelse(first[seq(1,nrow(first)-1,2),]<0,1,2) # 奇数轨偶数行根据奇数轨奇数行
      pattern[seq(4,n,4),]=ifelse(second[seq(1,nrow(second)-1,2),]>0,1,2) # 偶数轨偶数行根据偶数轨奇数行
    }
    sumtemp=matrix(nrow = n,ncol =nrow(path))
...
# 单纯双轨：没有反一反，530
R6810R_new_index_0=run(all_data,path,0);R6810R_new_index_0[[3]]
# plan11结果:548
R6810R_new_index_1=run(all_data,path,1);R6810R_new_index_1[[3]]
# 极端情况：前后5个
sort(R6810R_new_index_1[[4]][,4])[756:760];sort(R6810R_new_index_1[[4]][,4])[1:5]
for(i in 1:8){
  print(sum(R6810R_new_index_1[[4]][,i]>=0)/760)
}

# plan13:544
R6810R_new_index_2=run(all_data,path,2);R6810R_new_index_2[[3]]
# plan13-1:549
R6810R_new_index_21=run(all_data,path,3)
presient=function(R6810R_new_index){
  ...
  print(paste0('前、后5个结果：',str_c(sort(R6810R_new_index_21[[4]][,index],decreasing = T)[1:5],collapse = ' '),'；',str_c(sort(R6810R_new_index_21[[4]][,index])[1:5],collapse = ' ')))

}
presient(R6810R_new_index_21)
...
R6810R_new_index_3=run(all_data,path,2);R6810R_new_index_3[[3]]
presient(R6810R_new_index_3)

# plan20-1:
R6810R_new_index_4=run(all_data,path,2);R6810R_new_index_4[[3]]
presient(R6810R_new_index_4)
