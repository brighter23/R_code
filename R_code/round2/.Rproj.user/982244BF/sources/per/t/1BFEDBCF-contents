...
{
  library(stringr)
  formula=read.csv2('data\\original357formula.csv',header = F,stringsAsFactors = F)
  ...
  R3A=Formula(R3A,c1 = 1:3,c2=4:6);R3B=Formula(R3B,c1 = 1:3,c2=4:6)
  R5A=Formula(R5A,c1 = 1:3,c2=4:6);R5B=Formula(R5B,c1 = 1:3,c2=4:6)
  R7A=Formula(R7A,c1 = 1:3,c2=4:6);R7B=Formula(R7B,c1 = 1:3,c2=4:6)
  
  path_BP_original357=matrix(c(R3A,R5A,R7A, R3A,R5B,R7A, R3A,R5A,R7B, R3A,R5B,R7B, R3B,R5A,R7A, R3B,R5B,R7A, R3B,R5A,R7B, R3B,R5B,R7B),byrow = T,ncol = 6)
  colnames(path_BP_original357)=c('B_R3','P_R3','B_R5','P_R5','B_R7','P_R7')
}

c=c('data','all_data','path_BP_original357')
a=ls()
rm(list=(setdiff(a,c)))
gc()
library(stringr)
...
# new_quantize(path = path_BP_original357,set=data[1:7,1],Inverse=T) # 测试应该是正负相对
# 2.3 定义诊断连续负的长度：
...
# ================================================================
# 顺+不交叉：
plan1019201_original357=run(data = all_data,path = path_BP_original357,inverse = F,choice=1019201,rolling=T,reflection=F,cross = F)
present(plan1019201_original357)

plan101911_original357=run(data = all_data,path = path_BP_original357,inverse = F,choice=101911,rolling=T,reflection=F,cross = F)
present(plan101911_original357)
# 顺序+交叉（twist）：
plan1019201_original357_cross=run(data = all_data,path = path_BP_original357,inverse = F,choice=1019201,rolling=T,reflection=F,cross = T)
present(plan1019201_original357_cross)
