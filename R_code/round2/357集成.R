...
{
  library(stringr)
  formula=read.csv2('data\\357formula.csv',header = F,stringsAsFactors = F)
  R3A=formula[1:2,];R3B=formula[4:5,]
  R5A=formula[7:12,];R5B=formula[14:19,]
  R7A=formula[21:30,];R7B=formula[32:41,]
  Formula=function(R){
  ...
  R3A=Formula(R3A);R3B=Formula(R3B)
  R5A=Formula(R5A);R5B=Formula(R5B)
  R7A=Formula(R7A);R7B=Formula(R7B)
  
  path_BP=matrix(c(R3A,R5A,R7A, R3A,R5B,R7A, R3A,R5A,R7B, R3A,R5B,R7B, R3B,R5A,R7A, R3B,R5B,R7A, R3B,R5A,R7B, R3B,R5B,R7B),byrow = T,ncol = 6)
  colnames(path_BP)=c('B_R3','P_R3','B_R5','P_R5','B_R7','P_R7')
...
true_path0=function(dataset){
  ...
  t6=table(dataset[5:7])
  result6=paste0(t6[1],t6[2])
  t7=table(dataset[5:6])
  result7=paste0(t7[1],t7[2])
  t8=table(dataset[5:5])
...
true_path1=function(dataset){
  ...
  t6=table(dataset[c(1:2,7)])
  result6=paste0(t6[1],t6[2])
  t7=table(dataset[c(1:2)])
  result7=paste0(t7[1],t7[2])
  t8=table(dataset[c(1:1)])
  result8=paste0(t8[1],t8[2])
  comb3=paste0(result8,'-',result7,'-',result6)
  return(matrix(c(comb1,comb2,comb3),ncol = 3))
...
new_quantize=function(path,set,Inverse=F){
  ...
  if(ncol(path)==6){
    ...
    for (j in 1:nrow(path)) {
      ...
      r5[1,j]=ifelse(sparate_condition=='B'|sparate_condition==1,ifelse(str_detect(real_comb[1,2],path[j,'B_R5']),1,-1),ifelse(str_detect(real_comb[1,2],path[j,'P_R5']),1,-1))
    }
    r3=matrix(0,ncol = nrow(path),nrow = 1)
    for (j in 1:nrow(path)) {
      sparate_condition=ifelse(Inverse==T,set[1],set[5])
      r3[1,j]=ifelse(sparate_condition=='B'|sparate_condition==1,ifelse(str_detect(real_comb[1,3],path[j,'B_R3']),1,-1),ifelse(str_detect(real_comb[1,3],path[j,'P_R3']),1,-1))
    }
  }
  else if(ncol(path)==3){
    r7=matrix(0,ncol = nrow(path),nrow = 1)
    ...
    r5=matrix(0,ncol = nrow(path),nrow = 1)
    for (j in 1:nrow(path)) {
      r5[1,j]=ifelse(str_detect(real_comb[1,2],path[j,'R5']),1,-1)
    }
    r3=matrix(0,ncol = nrow(path),nrow = 1)
    for (j in 1:nrow(path)) {
      r3[1,j]=ifelse(str_detect(real_comb[1,3],path[j,'R3']),1,-1)
    }
  }
  return(r7+r5+r3)
} 
...
# 取数据：
{
  ...
  data_sort1=function(data=data,m,path=path){
    ...
    # 第三手：reflection+逆
    {
      ...
      for (i in seq(23,n,4)) {
        rejection=factor(c(reflaction[seq((i-2),(i-12),-2)],as.character(data[i,m])),levels = c('B',"P"))
  ...
  data_sort1_sequence1=function(data=data,m,path=path){
    ...
    # 第三手：reflection+顺
    {
      third_hand=matrix(ncol = nrow(path))
      for (i in seq(23,n,4)) {
        rejection=factor(c(reflaction[seq((i-12),(i-2),2)],as.character(data[i,m])),levels = c('B',"P"))
        third_hand_quantity=new_quantize(path,set = rejection)
        third_hand=rbind(third_hand,third_hand_quantity)
      }
      third_hand=third_hand[-1,]
...
# 错误版本4：分BP，从13手开始
plan1_11_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-1,choice = 11)
present(plan1_11_BP_thirteen)
plan1_13_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-1,choice = 13)
present(plan1_13_BP_thirteen)
plan1_131_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-1,choice = 131)
present(plan1_131_BP_thirteen)
...
plan2_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-2)
present(plan2_BP_thirteen) #plan13
plan2_1_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-2,choice = 1)
present(plan2_1_BP_thirteen)#plan13-1
plan2_2_BP_thirteen=run(data=all_data,path=path_BP,data_sort=-2,choice = 2)
present(plan2_2_BP_thirteen)#plan11
## 二、部分顺序部分逆序=================================================================
# 错误版本4：分BP，从21手开始
plan1_11_BP_sequence=run(data=all_data,path=path_BP,data_sort=10,choice = 11)
present(plan1_11_BP_sequence) 
plan1_13_BP_sequence=run(data=all_data,path=path_BP,data_sort=10,choice = 13)
present(plan1_13_BP_sequence)
plan1_131_BP_sequence=run(data=all_data,path=path_BP,data_sort=10,choice = 131)
present(plan1_131_BP_sequence)
# 错误版本4：分BP，从13手开始
plan1_11_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-10,choice = 11)
present(plan1_11_BP_sequence_thirteen)
plan1_13_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-10,choice = 13)
present(plan1_13_BP_sequence_thirteen)
plan1_131_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-10,choice = 131)
present(plan1_131_BP_sequence_thirteen)

# 错误plan2319:分BP，从21手开始
plan2_2_BP_sequence=run(data=all_data,path=path_BP,data_sort=20,choice = 2)
present(plan2_2_BP_sequence)#plan11
plan2_BP_sequence=run(data=all_data,path=path_BP,data_sort=20)
present(plan2_BP_sequence) #plan13
plan2_1_BP_sequence=run(data=all_data,path=path_BP,data_sort=20,choice = 1)
present(plan2_1_BP_sequence)#plan13-1

# 错误plan2319:分BP，从13手开始
plan2_2_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-20,choice = 2)
present(plan2_2_BP_sequence_thirteen)#plan11
plan2_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-20)
present(plan2_BP_sequence_thirteen) #plan13
plan2_1_BP_sequence_thirteen=run(data=all_data,path=path_BP,data_sort=-20,choice = 1)
present(plan2_1_BP_sequence_thirteen)#plan13-1


## 三、部分顺序部分逆序1（不同的地方顺序）=================================================================
# 错误版本4：分BP，从21手开始
plan1_11_BP_sequence1=run(data=all_data,path=path_BP,data_sort=11,choice = 11)
present(plan1_11_BP_sequence1) 
plan1_13_BP_sequence1=run(data=all_data,path=path_BP,data_sort=11,choice = 13)
present(plan1_13_BP_sequence1)
plan1_131_BP_sequence1=run(data=all_data,path=path_BP,data_sort=11,choice = 131)
present(plan1_131_BP_sequence1)
# 错误版本4：分BP，从13手开始
plan1_11_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-11,choice = 11)
present(plan1_11_BP_sequence1_thirteen)
plan1_13_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-11,choice = 13)
present(plan1_13_BP_sequence1_thirteen)
plan1_131_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-11,choice = 131)
present(plan1_131_BP_sequence1_thirteen)

# 错误plan2319:分BP，从21手开始
plan2_2_BP_sequence1=run(data=all_data,path=path_BP,data_sort=21,choice = 2)
present(plan2_2_BP_sequence1)#plan11
plan2_BP_sequence1=run(data=all_data,path=path_BP,data_sort=21)
present(plan2_BP_sequence1) #plan13
plan2_1_BP_sequence1=run(data=all_data,path=path_BP,data_sort=21,choice = 1)
present(plan2_1_BP_sequence1)#plan13-1

# 错误plan2319:分BP，从13手开始
plan2_2_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-21,choice = 2)
present(plan2_2_BP_sequence1_thirteen)#plan11
plan2_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-21)
present(plan2_BP_sequence1_thirteen) #plan13
plan2_1_BP_sequence1_thirteen=run(data=all_data,path=path_BP,data_sort=-21,choice = 1)
present(plan2_1_BP_sequence1_thirteen)#plan13-1


## 四、全部都是顺序=============================================================
# 错误版本4：分BP，从21手开始
plan1_11_BP_allsequence=run(data=all_data,path=path_BP,data_sort=12,choice = 11)
present(plan1_11_BP_allsequence)
plan1_13_BP_allsequence=run(data=all_data,path=path_BP,data_sort=12,choice = 13)
present(plan1_13_BP_allsequence)
plan1_131_BP_allsequence=run(data=all_data,path=path_BP,data_sort=12,choice = 131)
present(plan1_131_BP_allsequence)
# 错误版本4：分BP，从13手开始
plan1_11_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-12,choice = 11)
present(plan1_11_BP_allsequence_thirteen)
plan1_13_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-12,choice = 13)
present(plan1_13_BP_allsequence_thirteen)
plan1_131_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-12,choice = 131)
present(plan1_131_BP_allsequence_thirteen)

# 错误plan2319:分BP，从21手开始
plan2_BP_allsequence=run(data=all_data,path=path_BP,data_sort=22)
present(plan2_BP_allsequence) #plan13
plan2_1_BP_allsequence=run(data=all_data,path=path_BP,data_sort=22,choice = 1)
present(plan2_1_BP_allsequence)#plan13-1
plan2_2_BP_allsequence=run(data=all_data,path=path_BP,data_sort=22,choice = 2)
present(plan2_2_BP_allsequence)#plan11
# 错误plan2319:分BP，从13手开始
plan2_2_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-22,choice = 2)
present(plan2_2_BP_allsequence_thirteen)#plan11
plan2_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-22)
present(plan2_BP_allsequence_thirteen) #plan13
plan2_1_BP_allsequence_thirteen=run(data=all_data,path=path_BP,data_sort=-22,choice = 1)
present(plan2_1_BP_allsequence_thirteen)#plan13-1

## 五、正确版本的取数据===========================================================================
# 正确版本4：分BP
plan10_11_BP=run(data=all_data,path=path_BP,data_sort=100,choice = 11)
present(plan10_11_BP)
plan10_13_BP=run(data=all_data,path=path_BP,data_sort=100,choice = 13)
present(plan10_13_BP)
plan10_131_BP=run(data=all_data,path=path_BP,data_sort=100,choice = 131)
present(plan10_131_BP)

# 正确plan2319:分BP
plan20_BP=run(data=all_data,path=path_BP,data_sort=200)
present(plan20_BP) #plan13
plan20_1_BP=run(data=all_data,path=path_BP,data_sort=200,choice = 1)
present(plan20_1_BP)#plan13-1
plan20_2_BP=run(data=all_data,path=path_BP,data_sort=200,choice = 2)
